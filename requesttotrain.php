...
// Get trainer's ASHI permission
$trainerAshi = isset($trow['ashi']) ? $trow['ashi'] : 0;
...
<td colspan=\"20\" style=\"height: 1px;\"><a name='class".$crow["id"]."'></a></td>
</tr>
<tr>
<td>".$crow["id"]."</td>
<td>
...
// Fetch other classes with ASHI filter applied
$otherClassesSql = "select class.* from requesttotrain, class where done in ( -4, -6 ) and requesttotrain.trainerid = $tid and class.id = requesttotrain.classid and startdate > now() and classid <> '$id' and accepted = 1 and canceldate is null and class.id not in ( select classid from trainer_to_class where trainerid = $tid ) ";

if ($trainerAshi == 0) {
    $otherClassesSql .= " and code not like '%ASHI%' ";
}

$otherClassesSql .= " order by class.startdate";

$otherclasses = db_query_rows( $otherClassesSql, "id" );
$othergrouping = array();
