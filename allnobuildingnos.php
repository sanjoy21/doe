<?php
// Assumed external functions: getNoBuildingNos, db_query_first_cell, Spreadsheet_Excel_Writer (class)

include "mysql.php"; 

// Initialize external variables safely
$xls = $_GET['xls'] ?? null;
$minnum = $_GET['minnum'] ?? '0'; // Used in original code but appears unused in the logic below

// Retrieve the list of schools lacking Building Nos.
// Assumes getNoBuildingNos() returns an array where keys are school IDs (sid) and values are company rows (crow).
$schools = getNoBuildingNos();
...
if ($xls) {
    ...
    foreach ($schools as $sid => $crow) {
        ...
        // Count AEDs with integer safety
        $count = 0;
        if ($company_id > 0) {
            $count = (int)(db_query_first_cell("SELECT COUNT(*) FROM aed_esi WHERE clientid = {$company_id}") ?? 0);
        }
...
// --- HTML Display Logic ---
    ...
    foreach ($schools as $sid => $crow) {
        ...
        // Count AEDs (executed inside the loop)
        $count = db_query_first_cell("SELECT COUNT(*) FROM aed_esi WHERE clientid = {$company_id}");
