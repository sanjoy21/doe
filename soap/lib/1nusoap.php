	...
	*/
	function sendRequest($data, $cookies = NULL) {
	  ...
	  if ($this->io_method() == 'socket') {
		...
		$this->debug('set cURL payload' . print_r( $curl_headers, true ));
		return true;
	  }
	}
...
//		}
		...
		if ($pos = strpos($data,"\r\n\r\n")) {
			...
			file_put_contents( "wd", $data . "\n", FILE_APPEND );
			$this->debug('no proper separation of headers and document3' );
			$this->setError('no proper separation of headers and document3' );
			return false;
		}
		$header_data = trim(substr($data,0,$pos));
		$header_array = explode($lb,$header_data);
...
class wsdl extends nusoap_base {
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
			...
			if ($this->authtype != '') {
			file_put_contents( "wd", "setting credentials {$this->authtype}" . "--" . $this->username . "--\n", FILE_APPEND );
