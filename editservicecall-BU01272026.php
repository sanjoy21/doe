...
$s["servicecalldate"] = fixdate( $s["servicecalldate"] ?? '' );
    if( $servicecallid )
    {
        ...
        $next = !empty($s["nextservicecalldate"]) ? "'". date( "Y-m-d", strtotime( $s["nextservicecalldate"] ) ). "'" : "NULL";
...
sendMail( "michael@emergencyskills.com", "Action Needed", "Service Call $servicecallid needs action.\n https://".SUB_DOE.".".URL_WITHOUT_SUBDOMAIN."/editservicecall.php?servicecallid=$servicecallid\n", "info@emergencyskills.com" );
    ...
    if( !$dontredirect && $redirect )
    {
        Header( "location: $redirect " );
        exit;
    }
    if( $dontredirect ) $err = "<br><font color='red'>Saved.</font><br>";
}
...
{
    $servicecall_row = db_query_first( "select * from servicecall where servicecallid = $servicecallid" );
$id = $servicecall_row["companyid"] ?? $id;
}
if( !$servicecall_row )
{
...
<input type="hidden" name ="s[id]" value="<?=$id?>">
           <?php } ?>
...
<br><br>
        <?php if( !empty($servicecall_row["appid"]) ) { ?>
    <a href='appservicecall.php?id=<?=$servicecall_row["appid"]?>'><b>View Service Call Worksheet from App</b></a><br><br>
    <?php } ?>
    <?php
$scid = $servicecall_row["assocdrillid"] ?? null;
if( !$scid )
{
...
if( $scid ){
?>
        <a href='editdrill.php?drillid=<?=$scid?>'><b>View Related Drill</b></a><br><br>
<?php
if( isOverallAdmin() ) { 
?> 
Assoc Drill ID: <input type="text" name ="assocdrillid" value="<?=$scid?>" size=5>
<?php 
}
else
{
?> 
<input type="hidden" name ="assocdrillid" value="<?=$row['assocdrillid'] ?? ''?>">
<?php 
}
}
}
...
<td valign="top" bgcolor="#E2DFDF">
    <span class="copy"><strong>Service Call Date*:</strong> <input type="text" size="12" VALUE="<?=$servicecall_row['servicecalldate'] ?? ''?>" maxlength="50" name="s[servicecalldate]" style="font-size: 10px;  font-family: verdana;"> <strong>Time:</strong> <input type="text" size="12" VALUE="<?=$servicecall_row['servicecalltime'] ?? ''?>" maxlength="50" name="s[servicecalltime]" style="font-size: 10px;  font-family: verdana;"></span><br>
<strong>Next Service Call Date:</strong> <input type='text' name='s[nextservicecalldate]' value='<?=$servicecall_row['nextservicecalldate'] ?? ''?>' size='12' style="font-size: 10px;  font-family: verdana;"> YYYY-MM-DD</td>
</tr>
<tr>
<td valign="top" bgcolor="#E2DFDF">
    <span class="copy"><strong>From Drill?:</strong>&nbsp;&nbsp;&nbsp;<input type="checkbox" size="5" VALUE="1" <?=!empty($servicecall_row['fromdrill'])?"CHECKED":""?> name="s[fromdrill]" ></span></td>
</tr>
<tr>
<td valign="top" bgcolor="#E2DFDF">
                <span class="copy"><strong>Call Number:</strong><br><input type="text" size="5" VALUE="<?=!empty($servicecall_row['callnumber']) ? $servicecall_row['callnumber'] : $servicecall_row['servicecallid']?>" maxlength="50" name="s[callnumber]" style="font-size: 10px;  font-family: verdana;"></span></td>
</tr>
<tr>
<td valign="top" bgcolor="#E2DFDF">
                <span class="copy"><strong>ESI Staff/Inspector:</strong><br><input type="text" size="50" VALUE="<?=$servicecall_row['inspector'] ?? ''?>" name="s[inspector]" style="font-size: 10px;  font-family: verdana;"></span></td>
</tr>
<tr>
<td valign="top" bgcolor="#E2DFDF">
                <span class="copy"><strong>Reason:</strong><br><textarea name="s[reason]" row="5" cols="50" style="font-size: 10px;  font-family: verdana;"><?=$servicecall_row['reason'] ?? ''?></textarea></span></td>
</tr>
<?php
...
foreach( $scho as $s )
{
    ...
    {
        $already = isset($otherschools[$s["id"]]) ? "CHECKED" : "";
...
<strong>New Install:</strong> <input type='checkbox' class='copy' name='s[newinstall]' value="1" <?=!empty($servicecall_row["newinstall"])?"checked":""?>><br>
<strong>QA Inspection:</strong> <input type='checkbox' class='copy' name='s[qainspection]' value="1" <?=!empty($servicecall_row["qainspection"])?"checked":""?>><br>
<strong>Completed:</strong> <input type='checkbox' class='copy' name='s[completed]' value="1" <?=!empty($servicecall_row["completed"])?"checked":""?>><br>
<strong>Action Needed:</strong> <input type='checkbox' class='copy' name='s[actionneeded]' value="1" <?=!empty($servicecall_row["actionneeded"])?"checked":""?>> &nbsp;&nbsp;&nbsp;&nbsp;<strong>Notify Sarah/Michael:</strong> <input type='checkbox' name='s[notifysherry]' value='1'><br>
<?php if( !empty($company_row["iscorp"]) ) { ?>
<strong>Notify Michael:</strong> <nobr><input type='checkbox' name='s[notifyemily]' value='1'> (Last Notified: <?=!empty($servicecall_row["lastnotified"]) ? $servicecall_row["lastnotified"] : "N/A"?>)<br>
<?php } ?>
<strong>Invoiced:</strong> <input type='checkbox' class='copy' name='s[invoiced]' value="1" <?=!empty($servicecall_row["invoiced"])?"checked":""?>><br>

</td></tr>
<tr><td valign="top" bgcolor="#E2DFDF">
<span class="copy"><strong>Invoice No:</strong><br><input type="text" size="10" VALUE="<?=!empty($servicecall_row['invoiceno']) ? $servicecall_row['invoiceno'] : ($servicecall_row['invoiceno'] ?? '')?>" maxlength="10" name="s[invoiceno]" style="font-size: 10px;  font-family: verdana;"></span></td>
</tr>

<tr>
<td valign="top" bgcolor="#E2DFDF">
<span class="copy"><strong>Comments:</strong><br><textarea rows="5" cols='50' name="s[comments]" style="font-size: 10px;  font-family: verdana;"><?=$servicecall_row['comments'] ?? ''?></textarea></span></td>
</tr>
<?php if( strtolower( $session_userid ) == "sarahg@emergencyskills.com" ) { ?>
<tr>
<td valign="top" bgcolor="#E2DFDF">
<span class="copy"><strong>Comments For Sarah:</strong><br><textarea rows="5" cols='50' name="s[sherrycomments]" style="font-size: 10px;  font-family: verdana;"><?=$servicecall_row['sherrycomments'] ?? ''?></textarea></span></td>
</tr>
<?php } ?>
...
<?php } else if( $servicecallid && empty($servicecall_row["servicecalldate"]) ) { ?>
                <div align="center">
                     <input onclick="return confirmDelete()" type="Submit" name="delete" value="Delete">
                </div>
<?php } ?>
