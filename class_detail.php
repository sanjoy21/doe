...
// less than 36 hours
if (1 == 1) {  // This logic is set by Sanjoy Dey to always show the link for all classes based on clients demand.
echo '<br><a href="#" onclick="showConfirmationModal(event, \'' . $id . '\')">Request To Cancel</a>';

echo '<div id="cancelModal" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:white; padding:20px; border:2px solid #ccc; z-index:1000; box-shadow:0 0 10px rgba(0,0,0,0.5);">';
echo '<p>Please confirm cancellation:</p>';
echo '<label>';
echo '<input type="checkbox" id="modalCheckbox"> ';
echo 'I acknowledge that I am removing myself from this class and that I will text the management group as the last step to officially being removed from the class.';
echo '</label><br><br>';
echo '<button onclick="confirmCancellation(\'' . $id . '\')">Confirm Cancellation</button>';
echo '<button onclick="closeModal()">Cancel</button>';
echo '</div>';

echo '<div id="modalOverlay" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:999;"></div>';

echo '<script>
function showConfirmationModal(event, id) {
...
function confirmCancellation(id) {
    if (!document.getElementById("modalCheckbox").checked) {
        alert("Please check the acknowledgement box.");
        return;
    }
    window.location.href = "class_detail.php?id=" + id + "&sos=36";
}
</script>';
  }
...
//         return;
//     }
//     window.location.href = "class_detail.php?id=" + id + "&sos=1";
// }
// </script>';
//   }
...
<!-- The below table was added later based on clients demand -->
 <?php 
 $class_cancel_request = db_query_rows("select * from sostrainers where classid = $id");
 if(count($class_cancel_request) && isOverallAdmin()) {
 ?>
  <table class="classdetailtable">
<tr>
 <td valign="top" align="right" width="20%"><span class="copy"><strong>Request to Cancel:</strong></span></td>
 <td valign="top" width="80%"><span class="copy">&nbsp;
<?php foreach ($class_cancel_request as $trainerid => $trow) {
?>
 <?php if (isOverallAdmin()) 
 { 
  echo getFullname($trow["trainerid"]);
  echo " (".$trow["dateadded"].")"; ?><Br>
 <?php } ?>
 <br>
<?php } ?>
  </span></td>
  ...
  <?php } ?>
  <!-- The above table was added later based on clients demand -->
